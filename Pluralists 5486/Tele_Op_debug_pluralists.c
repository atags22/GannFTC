...
#pragma config(Motor,  mtr_S1_C1_1,     blockLift,     tmotorTetrix, openLoop)
#pragma config(Motor,  mtr_S1_C1_2,     wheelLeft,     tmotorTetrix, openLoop, reversed)
#pragma config(Motor,  mtr_S1_C2_1,     omniLeft,      tmotorTetrix, PIDControl, reversed, encoder)
#pragma config(Motor,  mtr_S1_C2_2,     motorG,        tmotorTetrix, PIDControl, encoder)
#pragma config(Motor,  mtr_S2_C2_1,     wheelRight,    tmotorTetrix, openLoop)
#pragma config(Motor,  mtr_S2_C2_2,     omniRight,     tmotorTetrix, openLoop)
...
#pragma config(Servo,  srvo_S1_C3_3,    collectorTiltLeft,    tServoStandard)
#pragma config(Servo,  srvo_S1_C3_4,    dispenserTiltBack,    tServoContinuousRotation)
#pragma config(Servo,  srvo_S1_C3_5,    dispenserTiltFront,   tServoContinuousRotation)
...
#pragma config(Servo,  srvo_S2_C1_3,    dispenserTop,         tServoStandard)
...
#define SERVO_LEFT_COLLECT_OPEN_POS 256
#define SERVO_LEFT_COLLECT_CLOSE_POS 0
#define SERVO_RIGHT_COLLECT_OPEN_POS 256
#define SERVO_RIGHT_COLLECT_CLOSE_POS 0

#define SERVO_LEFT_COLLECT_TILT_UP_POS 70
#define SERVO_LEFT_COLLECT_TILT_DOWN_POS 256
#define SERVO_RIGHT_COLLECT_TILT_UP_POS 170 //186
#define SERVO_RIGHT_COLLECT_TILT_DOWN_POS 0

#define SERVO_DISPENSE_TOP_OPEN_POS 0
#define SERVO_DISPENSE_TOP_CLOSE_POS SERVO_MAX

#define SERVO_FRONT_DISPENSE_TILT_OPEN_POS 256
#define SERVO_FRONT_DISPENSE_TILT_CLOSE_POS 256
...
void updateDriveMotors() {
	...
	int joy1Y = .5 * -joystick.joy1_x1;
	int joy1X = .5 * joystick.joy1_y1;
	...
	motor[omniRight] = joy1Y;
	motor[omniLeft] = -joy1Y;
...
void debugDispenserServos() {
	...
	servo[dispenserTiltFront] = 256;
	servo[dispenserTiltBack] = 0;
	servo[dispenserTop]=SERVO_DISPENSE_TOP_CLOSE_POS;
	wait1Msec(1200);
	servo[dispenserTiltFront] = 127;
	servo[dispenserTiltBack] = 127;
	wait1Msec(MTR_DEBUG_TIME_MS);
	servo[dispenserTiltFront]=0;
	servo[dispenserTiltBack]=256;
	wait1Msec(MTR_DEBUG_TIME_MS*2.5);
	servo[dispenserTiltFront] = 127;
	servo[dispenserTiltBack] = 127;
	wait1Msec(MTR_DEBUG_TIME_MS);
}
