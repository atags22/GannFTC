...
bool letswait = true; //Delay at start of match?
...
void stoppingPlaceBlock()
{
	...
	Stop();
	wait10Msec(80);
	servoChangeRate[AutoBlockator] -= 2;
	servo[AutoBlockator] -= 87;
	wait10Msec(90);
	servo[AutoBlockator] += 87;
	servoChangeRate[AutoBlockator] += 2;
}
	...
	while (true)
	{
		...
		switch (Stage)
		{
			...
			case 1:
				...
				stageSide(STAGE_1, 0.2868, 60); //InverseTangent(42.5/13.5). Plus Pi. Bad angle drive algorithm. **UPDATE** InverseTangent(42.5/13). The flag raiser. Corner.
				break;
