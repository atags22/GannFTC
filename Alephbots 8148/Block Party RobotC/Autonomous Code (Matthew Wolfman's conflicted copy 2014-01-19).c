...
int RouteXCoord[] = { 1, 1, 1, 1, -1 }; // X coordinates for pre-programmed route
int RouteYCoord[] = { 1, 2, 3, 4, -1 }; // Y coordinates for pre-programmed route
int CurrentYCoord = 0;
int CurrentXCoord = 0;
...
bool EndOfRoute()
{
	if ((RouteXCoord[CurrentRouteXIndex] == -1) && (RouteYCoord[CurrentRouteYIndex] == -1)
	{
		return true;
	}
	else
...
task main()
{
	...
	while(true)
	{
		if (IsEndOfRoute() == true)
		{
			MoveRobot(0);  // at end of route, stop robot
		}
		else
		{
			// if next route coordinate is not in the same direction then turn the robot
		// turn robot
		}
